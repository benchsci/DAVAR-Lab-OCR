...
from mmdet.core.visualization.image import EPS, color_val_matplotlib
...
def imshow_det_bboxes2(img,
                      ...
                      out_file=None):
    """
    This function was modified from mmdet.core.visualization.immshow_det_bboxes
    ...
    if labels.shape[0] > 0:
        if mask_color is None:
            ...
            np.random.seed(42)
            mask_colors = [
                tuple(np.random.randint(0, 256, 3, dtype=np.uint8))
                for _ in range(max(labels) + 1)
            ]
            mask_colors = [color_val_matplotlib(c) for c in mask_colors]
        else:
            # specify  color
            mask_colors = [
    ...
    for i, (bbox, label) in enumerate(zip(bboxes, labels)):
        ...
        color.append(mask_colors[labels[i]])

...
data = dict(
	...
	train=dict(
		type=dataset_type,
		ann_file=DAVAROCR_PATH + '/Datalist/500K_train_datalist_sample.json',
		img_prefix=DATASET_PATH + '/Images/',
		pipeline=train_pipeline,
		ann_prefix=DATASET_PATH + '/Annos/',
        classes_config=DATASET_PATH + '/Datalist/classes_config.json',
		classes=('abstract', 'author', 'caption', 'date', 'equation', 'figure', 'footer', 'list', 'paragraph',
				 'reference', 'section', 'table', 'title'),
	    max_num=2048),
	val=dict(
		type=dataset_type,
        ann_file=DATASET_PATH + '/Datalist/500K_dev_datalist_sample.json',
        img_prefix=DATASET_PATH + '/Images/',
...
log_level = 'INFO'
work_dir=DAVAROCR_PATH + '/VSR/DocBank/log/docbank_x101/'
load_from = DAVAROCR_PATH + '/VSR/common/mask_rcnn_x101_64x4d_fpn_1x_coco_20200201-9352eb0d_with_semantic.pth'
resume_from = None
workflow = [('train', 1)]

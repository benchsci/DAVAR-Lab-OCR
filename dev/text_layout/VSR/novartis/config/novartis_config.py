...
train_pipeline = [
	...
	dict(type='CharTokenize', vocab=DAVAROCR_PATH + "/datalist/PubLayNet/char_vocab.txt", targets=['gt_ctexts']),
...
test_pipeline = [
	...
	dict(
		...
		transforms=[
			...
			dict(type='Pad', size_divisor=32),
			# CharTokenizer for chargrid
			dict(type='CharTokenize', vocab=DAVAROCR_PATH + "/datalist/PubLayNet/char_vocab.txt",
			     targets=['gt_ctexts']),
			dict(type='MMLAFormatBundle'),
...
log_level = 'INFO'
work_dir=DAVAROCR_PATH + '/VSR/novartis/log/'
